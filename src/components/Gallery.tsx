...
export default function Gallery() {
  return (
    <section id="gallery" className="bg-[#0B1320]">
      {/* Header */}
      <div className="text-center py-24 px-4 sticky top-0 z-10 bg-[#0B1320]">
        <p className="text-[#8A2BE2] text-sm font-semibold tracking-widest uppercase mb-3">
          Inside the Shop
        </p>
        <h2
          className="text-6xl sm:text-7xl lg:text-8xl text-[#F2F6FF] mb-4"
          style={{ fontFamily: "var(--font-bebas), cursive" }}
        >
          See What&apos;s In Store
        </h2>
        <p className="text-[#A9B4C7] text-lg mb-2">
          Real photos from our locations across UT, CO, ID & WY.
        </p>
        <p className="text-[#A9B4C7]/60 text-sm animate-bounce mt-6">
          Scroll down
        </p>
      </div>

      {/* Sticky scroll gallery — 3-column parallax */}
      <div className="grid grid-cols-12 gap-2 px-2 sm:px-4 pb-8">
        {/* Left column — scrolls normally */}
        <div className="col-span-4 grid gap-2">
          {PHOTOS_LEFT.map((photo, i) => (
            <figure key={`l-${i}`} className="w-full overflow-hidden rounded-lg">
              <img
                src={photo}
                alt={`Xvertuz store ${i + 1}`}
                className="w-full h-80 sm:h-96 object-cover transition-all duration-500 hover:scale-105"
                loading="lazy"
              />
            </figure>
          ))}
        ...
        {/* Center column — STICKY, locked in viewport */}
        <div className="sticky top-0 h-screen col-span-4 grid grid-rows-3 gap-2">
          {PHOTOS_CENTER.map((photo, i) => (
            <figure key={`c-${i}`} className="w-full h-full overflow-hidden rounded-lg">
              <img
                src={photo}
                alt={`Xvertuz featured ${i + 1}`}
        ...
        <div className="col-span-4 grid gap-2">
          {PHOTOS_RIGHT.map((photo, i) => (
            <figure key={`r-${i}`} className="w-full overflow-hidden rounded-lg">
              <img
                src={photo}
                alt={`Xvertuz store ${i + 5}`}
                className="w-full h-80 sm:h-96 object-cover transition-all duration-500 hover:scale-105"
                loading="lazy"
              />
            </figure>
